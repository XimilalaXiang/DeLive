...
- [x] **12 条 ASR 路径统一在一个 UI 里** — Soniox、火山引擎、ElevenLabs、Mistral AI、Gladia、Deepgram、AssemblyAI、Cloudflare Workers AI、硅基流动、Groq、本地 OpenAI-compatible、本地 `whisper.cpp`
...
- [x] **AI 复盘工作台（Review Desk）** — 全页视图，动画标签栏导航（Transcript、AI 纠错、Overview、AI 分析、Chat、Mind Map）
- [x] **AI 转录纠错** — 双模式：直接纠错（流式输出纠正结果）和先检测后纠错（先检测问题，用户确认后再纠正）；并排对比视图；纠错文本导出
- [x] **AI 后处理智能文本源选择** — 有纠错文本时自动使用；支持用户偏好设置（自动 / 始终原始 / 始终纠错后），各 AI 标签页实时状态横幅
...
- [x] **文件转录** — 上传音频/视频文件，使用 10 种 ASR 引擎离线转录为带时间戳的文本，支持说话人分离和多语言检测
...
| Provider | 类型 | 传输模式 | 音频路径 | 文件转录 | 亮点 |
|----------|------|----------|----------|----------|------|
| **Soniox V4** | 云端 | 实时流式 | `MediaRecorder` (`webm/opus`) → WebSocket | 支持 | token 级实时转录、实时翻译、双语字幕、多发言人识别 |
| **火山引擎** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | 中文场景友好；代理在 Electron 侧补齐必须 Header |
| **ElevenLabs** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | Scribe v2 Realtime；99 种语言含中文；音频以 base64 JSON 发送 |
| **Mistral AI** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | Voxtral Realtime，经 `wss://api.mistral.ai`；代理注入 `Authorization` Header |
| **Gladia** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | Solaria-1 实时；100+ 种语言；<300ms 延迟；代理处理会话初始化和 `x-gladia-key` 鉴权 |
| **Deepgram** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | Nova-3 / Nova-2 流式 ASR；代理注入 `Authorization: Token` Header |
| **AssemblyAI** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | 支持 | Universal-3 Pro 流式；最适合英语和多语言内容 |
| **Cloudflare Workers AI** | 云端 | 窗口批处理重转写 | `AudioWorklet` PCM16 → WAV → REST | 支持 | 基于 Whisper；低成本含免费额度；VAD 过滤和反幻觉 |
| **硅基流动** | 云端 | 窗口批处理重转写 | `AudioWorklet` PCM16 → WAV → REST | 支持 | SenseVoice、TeleSpeech、Qwen Omni 等模型路径 |
| **Groq** | 云端 | 窗口批处理重转写 | `AudioWorklet` PCM16 → WAV → REST | 支持 | Whisper `large-v3-turbo` / `large-v3` 准实时会话更新 |
...
`npm run dev` 会同时启动 Vite 和 Electron。Provider 代理（火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）已内置在 Electron 主进程中，正常桌面开发不需要单独后端。
...
- **AI 纠错标签页**：双模式纠错（直接纠错 / 先检测后纠错），流式输出带实时进度，并排对比视图，纠错文本导出 TXT / Markdown
- **Overview 标签页**：AI briefing — 摘要、行动项、关键词、章节、标题/标签建议，一键应用
- **AI 分析标签页**：详细 AI 分析，带文本源状态横幅显示当前使用原始或纠错后文本
...
| Provider 抽象层 | `frontend/src/providers/registry.ts`, `frontend/src/providers/implementations/*` | 把 12 个后端统一到同一套 contract 和 capability 模型。 |
...
| Review Desk UI | `frontend/src/components/review/SessionTabBar.tsx`, `frontend/src/components/review/SessionHeader.tsx`, `frontend/src/components/review/OverviewTab.tsx`, `frontend/src/components/review/TranscriptTab.tsx`, `frontend/src/components/review/CorrectionTab.tsx`, `frontend/src/components/review/AiTab.tsx`, `frontend/src/components/review/ChatTab.tsx`, `frontend/src/components/review/MindMapTab.tsx`, `frontend/src/components/review/TextSourceBanner.tsx`, `frontend/src/components/review/MarkdownRenderer.tsx` | 动画标签栏（含键盘导航）、会话头部（多格式导出 TXT/Markdown/SRT/VTT）、AI 纠错流式对比、文本源状态横幅、各标签页视图、GFM Markdown 渲染（含语法高亮）和思维导图编辑。 |
...
| 共享契约层 | `shared/electronApi.ts`, `electron/preload.ts`, `shared/volcProxyCore.ts`, `shared/mistralProxyCore.ts`, `shared/deepgramProxyCore.ts`, `shared/assemblyaiProxyCore.ts`, `shared/elevenlabsProxyCore.ts`, `shared/gladiaProxyCore.ts` | 定义 renderer 与 main 的类型化桥接接口，以及各 Provider 代理共享协议辅助逻辑。 |
| 调试与发布支持 | `server/`, `scripts/`, `.github/workflows/release.yml`, `.github/workflows/ci.yml`, `.github/workflows/test-build.yml` | 独立代理调试、图标/运行时预置脚本、持续集成、测试构建和 tag 触发的多平台构建发布。 |
...
| Provider 层 | 注册表 + 12 个实现 | 同时覆盖实时云端、窗口批处理云端、本地服务和本地 runtime。 |
| Electron 服务 | 内置多 Provider 代理（火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）、本地 runtime 控制器、safe-storage IPC、diagnostics IPC | 提供浏览器环境无法直接完成的能力。 |
...
2. **Provider 代理**：正常桌面使用无需单独后端，Electron 内置了火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia 的 WebSocket 代理。
