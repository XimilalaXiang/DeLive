...
| 实时转录 | 字幕悬浮窗 | MCP 集成 |
|:---:|:---:|:---:|
| 多 Provider ASR 实时转录 | 可拖拽的置顶字幕悬浮窗 | 外部 AI 工具通过 MCP 协议访问 DeLive |
...
| AI 概览 | AI 对话 | 思维导图 |
|:---:|:---:|:---:|
| 摘要、行动项、关键词与章节 | 多线程对话，带引用片段 | 从转录内容自动生成思维导图 |
| <img width="300" src="assets/screenshot-ai-overview.png" alt="AI 概览" /> | <img width="300" src="assets/screenshot-ai-chat.png" alt="AI 对话" /> | <img width="300" src="assets/screenshot-mindmap.png" alt="思维导图" /> |
...
- [x] **10 条 ASR 路径统一在一个 UI 里** — Soniox、火山引擎、Groq、硅基流动、Mistral AI、Deepgram、AssemblyAI、ElevenLabs、本地 OpenAI-compatible、本地 `whisper.cpp`
...
| **Mistral AI** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | Voxtral Realtime，经 `wss://api.mistral.ai`；代理注入 `Authorization` Header |
| **Deepgram** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | Nova-3 / Nova-2 流式 ASR；代理注入 `Authorization: Token` Header |
| **AssemblyAI** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | Universal-3 Pro 流式；最适合英语和多语言内容 |
| **ElevenLabs** | 云端 | 实时流式 | `AudioWorklet` PCM16 → 内置代理 → WebSocket | Scribe v2 Realtime；90+ 种语言含中文；音频以 base64 JSON 发送 |
| **本地 OpenAI-compatible** | 本地服务 | 窗口批处理重转写 | `MediaRecorder` (`webm/opus`) → `/v1/audio/transcriptions` | 适配 Ollama 或其他兼容网关，支持服务/模型探测和可选 Ollama 拉取 |
...
当前测试状态：**29 个测试文件、266 个测试用例全部通过**，覆盖 Provider 配置、转录状态/稳定化、字幕导出、会话生命周期与仓储、存储、云备份、Open API IPC 响应、AI 后处理解析以及转录文本源解析。
