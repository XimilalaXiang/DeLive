...
| 渲染层应用 | `frontend/src/App.tsx`, `frontend/src/components/*`, `frontend/src/i18n/*` | 主界面、设置、录制、历史、预览和字幕控制 UI；工作区视图（Live / Review Desk / Settings）由 Zustand 驱动。 |
...
| 会话智能层 | `frontend/src/stores/sessionStore.ts`, `frontend/src/services/aiPostProcess.ts`, `frontend/src/components/ReviewDeskView.tsx`, `frontend/src/components/PreviewModal.tsx` | 会话持久化、自动保存/恢复、AI briefing、问答、思维导图、标签和 speaker 名称编辑。 |
