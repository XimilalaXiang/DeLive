...
PC が音を再生できるなら、DeLive はそのシステム音声を取り込み、選択した ASR バックエンドへ送り、文字起こし結果をローカルに保存します。レビュー・検索・エクスポートまで一つのアプリで完結します。
...
- **履歴とエクスポート**：タグ、検索、TXT / SRT / VTT エクスポート、ローカルデータのバックアップ。
...
Vitest で 129 のユニットテストを実行。Provider 設定、字幕エクスポート、文字起こし安定化、ストレージユーティリティ、BaseASRProvider イベントシステムをカバーしています。
