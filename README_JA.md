...
- [x] **12 種の ASR バックエンドを統一 UI で切り替え** — Soniox、Volcengine、ElevenLabs、Mistral AI、Gladia、Deepgram、AssemblyAI、Cloudflare Workers AI、SiliconFlow、Groq、ローカル OpenAI-compatible、ローカル `whisper.cpp`
...
- [x] **AI レビューデスク（Review Desk）** — フルページワークスペース、アニメーション付きタブ（Transcript、AI 校正、Overview、AI 分析、Chat、Mind Map）
- [x] **AI 文字起こし校正** — 2 モード：クイック修正（ストリーミング直接校正）とレビュー＆修正（問題検出後にユーザー確認して修正）；サイドバイサイド差分表示；校正済みテキストエクスポート
- [x] **AI 後処理スマートテキストソース選択** — 校正済みテキストがあれば自動使用；ユーザー設定（自動 / 常に原文 / 常に校正済み）でカスタマイズ可能、各 AI タブにリアルタイムステータスバナー
...
- [x] **ファイル文字起こし** — 音声/動画ファイルをアップロードし、10 種の ASR エンジンでタイムスタンプ付きテキストに変換。話者識別と多言語検出をサポート
...
| プロバイダー | 種別 | 転送モード | 音声経路 | ファイル | 特徴 |
|--------------|------|------------|----------|----------|------|
| **Soniox V4** | クラウド | リアルタイムストリーミング | `MediaRecorder` (`webm/opus`) → WebSocket | 対応 | トークン単位リアルタイム文字起こし、リアルタイム翻訳、バイリンガル字幕、話者識別 |
| **Volcengine** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | 中国語向け最適化。プロキシが Electron 側で必要なヘッダーを補完 |
| **ElevenLabs** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | Scribe v2 Realtime；99 言語（中国語含む）；音声は base64 JSON で送信 |
| **Mistral AI** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | Voxtral Realtime（`wss://api.mistral.ai`経由）；プロキシが `Authorization` ヘッダーを注入 |
| **Gladia** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | Solaria-1 リアルタイム；100+ 言語；<300ms レイテンシ；プロキシがセッション初期化と `x-gladia-key` 認証を処理 |
| **Deepgram** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | Nova-3 / Nova-2 ストリーミング ASR；プロキシが `Authorization: Token` ヘッダーを注入 |
| **AssemblyAI** | クラウド | リアルタイムストリーミング | `AudioWorklet` PCM16 → 内蔵プロキシ → WebSocket | 対応 | Universal-3 Pro ストリーミング；英語と多言語コンテンツに最適 |
| **Cloudflare Workers AI** | クラウド | ウィンドウバッチ再文字起こし | `AudioWorklet` PCM16 → WAV → REST | 対応 | Whisper ベース；低コスト（無料枠あり）；VAD フィルタリングとアンチハルシネーション |
| **SiliconFlow** | クラウド | ウィンドウバッチ再文字起こし | `AudioWorklet` PCM16 → WAV → REST | 対応 | SenseVoice、TeleSpeech、Qwen Omni 等のモデルパス |
| **Groq** | クラウド | ウィンドウバッチ再文字起こし | `AudioWorklet` PCM16 → WAV → REST | 対応 | Whisper `large-v3-turbo` / `large-v3` 準リアルタイムセッション更新 |
...
- いずれかのプロバイダーパス：
  ...
  - **ElevenLabs**：[elevenlabs.io](https://elevenlabs.io) の API Key
  - **Mistral AI**：[mistral.ai](https://mistral.ai) の API Key
  - **Gladia**：[gladia.io](https://app.gladia.io) の API Key
  - **Deepgram**：[deepgram.com](https://deepgram.com) の API Key
  - **AssemblyAI**：[assemblyai.com](https://www.assemblyai.com) の API Key
  - **Cloudflare Workers AI**：[dash.cloudflare.com](https://dash.cloudflare.com) の API Token と Account ID
...
`npm run dev` で Vite と Electron が同時に起動します。プロバイダープロキシ（Volcengine、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）は Electron メインプロセスに組み込まれているため、通常のデスクトップ開発では別途バックエンドは不要です。
...
- **AI 校正タブ**：2 モード校正（クイック修正 / レビュー＆修正）、リアルタイム進捗付きストリーミング出力、サイドバイサイド差分比較、校正済みテキストの TXT / Markdown エクスポート
- **Overview タブ**：AI ブリーフィング — 要約、アクションアイテム、キーワード、チャプター、タイトル/タグ提案、ワンクリック適用
- **AI 分析タブ**：詳細な AI 分析、テキストソースステータスバナーで原文または校正済みテキストの使用状況を表示
...
| Review Desk UI | `frontend/src/components/review/SessionTabBar.tsx`, `frontend/src/components/review/SessionHeader.tsx`, `frontend/src/components/review/OverviewTab.tsx`, `frontend/src/components/review/TranscriptTab.tsx`, `frontend/src/components/review/CorrectionTab.tsx`, `frontend/src/components/review/AiTab.tsx`, `frontend/src/components/review/ChatTab.tsx`, `frontend/src/components/review/MindMapTab.tsx`, `frontend/src/components/review/TextSourceBanner.tsx`, `frontend/src/components/review/MarkdownRenderer.tsx` | アニメーション付きタブバー（キーボードナビ対応）、セッションヘッダー（マルチフォーマットエクスポート TXT/Markdown/SRT/VTT）、AI 校正ストリーミング差分、テキストソースステータスバナー、各タブビュー、GFM Markdown レンダリング（シンタックスハイライト）、マインドマップ編集。 |
...
| 共有コントラクト | `shared/electronApi.ts`, `electron/preload.ts`, `shared/volcProxyCore.ts`, `shared/mistralProxyCore.ts`, `shared/deepgramProxyCore.ts`, `shared/assemblyaiProxyCore.ts`, `shared/elevenlabsProxyCore.ts`, `shared/gladiaProxyCore.ts` | renderer と main 間の型付きブリッジ定義、各プロバイダープロキシ共有プロトコルヘルパー。 |
| デバッグ・リリース | `server/`, `scripts/`, `.github/workflows/release.yml`, `.github/workflows/ci.yml`, `.github/workflows/test-build.yml` | 単体プロキシデバッグ、アイコン/runtime ステージングスクリプト、継続的インテグレーション、テストビルド、タグトリガーのマルチプラットフォームビルド。 |
...
| プロバイダー層 | レジストリ + 12 の実装 | リアルタイムクラウド、バッチクラウド、ローカルサービス、ローカル runtime を統一。 |
| Electron サービス | 内蔵マルチプロバイダープロキシ（Volcengine、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）、ローカル runtime コントローラー、safe-storage IPC、diagnostics IPC | ブラウザ環境では直接行えない機能を提供。 |
...
2. **プロバイダープロキシ**：通常のデスクトップ利用では別プロセスのバックエンドは不要。Electron が Volcengine、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia 向け WebSocket プロキシを内蔵。
