...
[主な機能](#-主な機能) • [プロジェクト構成](#-プロジェクト構成) • [システムアーキテクチャ](#-システムアーキテクチャ) • [対応 ASR プロバイダー](#-対応-asr-プロバイダー) • [クイックスタート](#-クイックスタート)
...
- **デスクトップ向けのシステム音声キャプチャ**。ブラウザ動画、配信、会議、講座、ポッドキャストなど、システム音声を共有できるものなら何でも取り込み可能。
- **6 つの ASR バックエンドを統一 UI で切り替え**。Soniox、Volcengine、Groq、SiliconFlow、ローカル OpenAI-compatible、ローカル `whisper.cpp`。
- **プロバイダーに応じた音声パイプライン自動切換**。バックエンド要件に応じて `MediaRecorder` と `AudioWorklet` PCM16 を使い分け。
- **1 つのアプリで 3 種の実行モード**。リアルタイムストリーミング、ウィンドウバッチ再文字起こし、Electron 管理のローカル runtime。
- **セッションライフサイクル管理**。ドラフトセッション、録音中自動保存、中断セッションの次回起動時復元、完了済み履歴管理。
- **フローティング字幕ウィンドウ**。常に最前面の独立ウィンドウ。原文 / 翻訳 / バイリンガルモード、ドラッグ/ロック、スタイルカスタマイズに対応。
- **Soniox 専用バイリンガル・話者認識機能**。リアルタイム翻訳、バイリンガル字幕、話者識別トークン、話者別セッションプレビュー。
- **専用 AI レビューデスク（Review Desk）**。フルページワークスペース（モーダルではない）。スライドアニメーション付きタブバーとキーボード矢印キー操作に対応し、Overview・Transcript・Chat・Mind Map の 4 タブを備える。
- **リッチ AI チャット（Chat タブ）**。マルチスレッド会話、GFM Markdown レンダリング（シンタックスハイライト付きコードブロック・ワンクリックコピー）、ユーザー/AI アバター、ホバー時のコピー/再生成アクション、思考中ドットアニメーション、自動リサイズコンポーザー（Enter で送信）、スクロール復帰フローティングボタン、スレッド単位の削除。
- **構造化 AI ブリーフィング**。要約、アクションアイテム、キーワード、チャプター、タイトル/タグ提案、引用付き Q&A — すべてセッションに永続化。
- **マインドマップ**。Markmap 互換 Markdown を生成し、その場で編集可能。Review Desk から直接 SVG / PNG エクスポート。
- **充実の Transcript タブ**。左ガターにタイムスタンプ、カラーコード付き話者バッジ、連続同一話者の自動マージ、ホバーハイライト。
- **ローカルモデルワークフロー**。ローカルサービス検出、インストール済みモデル一覧、Ollama ワンクリック pull、`whisper.cpp` バイナリ / モデルのインポートとダウンロード。
- **ローカルファースト永続化**。セッション、タグ、設定は IndexedDB / localStorage に保存。秘密情報は OS 対応時に Electron `safeStorage` で暗号化保存。
- **共有デザインシステム**。コンポーザブル UI プリミティブ（Button、Badge、Switch、EmptyState、StatusIndicator、DialogShell）。5 テーマ（ライト/ダーク）で `warning`/`success`/`info` セマンティックカラートークンに対応。
- **デスクトップ統合**。システムトレイ、グローバルショートカット、自動起動、アップデートチェック、診断エクスポート、ソースピッカー、型付き preload API。
- **セキュリティ強化**。信頼ウィンドウ IPC 検証、CSP インジェクション、ナビゲーションガード、パスホワイトリスト、診断情報マスキング、秘密情報暗号化保存。
...
| レンダラーアプリ | `frontend/src/App.tsx`, `frontend/src/components/*`, `frontend/src/i18n/*` | メイン UI、設定、録音、履歴、プレビュー、字幕コントロール。ワークスペースビュー（Live / Review Desk / Settings）は Zustand で駆動。 |
...
| セッションインテリジェンス | `frontend/src/stores/sessionStore.ts`, `frontend/src/services/aiPostProcess.ts`, `frontend/src/components/ReviewDeskView.tsx`, `frontend/src/components/PreviewModal.tsx` | セッション永続化、自動保存/復元、AI ブリーフィング、Q&A、マインドマップ、タグ、話者名編集。 |
...
| レンダラー層 | React UI、Zustand stores、履歴/プレビューワークスペース、設定パネル | 録音フロー、設定、セッションレビュー、ユーザー操作を担当。 |
...
現在のテスト状況：**22 個のテストファイル、184 個のテストケースがすべてパス**。プロバイダー設定、文字起こし状態/安定化、字幕エクスポート、セッションライフサイクルとリポジトリ、ストレージ、AI 後処理パースをカバーしています。
...
完了済みセッションは専用のフルページ Review Desk で振り返り可能：

- **AI ブリーフィング**：要約、アクションアイテム、キーワード、チャプター、タイトル提案、タグ提案
- **セッション Q&A**：現在のセッションについてのみ質問。回答に短い引用を含めることが可能
- **マインドマップ**：Markmap 互換 Markdown を生成し、編集後に SVG / PNG エクスポート
- **メタデータ操作**：提案されたタイトル/タグのワンクリック適用、diarization セッションの話者名変更
...
- セッションのリネーム、タグ付け、検索、TXT / SRT / VTT エクスポートに対応。
- 録音ドラフトは自動保存。アプリ中断時は次回起動時に未完了セッションを復元可能。
- すべてのローカルデータのインポート/エクスポートに対応（バックアップ・移行用）。
- 診断エクスポートでマスキング済み JSON バンドル（システム情報 + 最近のログ）を生成。トラブルシューティングに活用可能。
