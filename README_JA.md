...
- [x] **10 種の ASR バックエンドを統一 UI で切り替え** — Soniox、Volcengine、Groq、SiliconFlow、Mistral AI、Deepgram、AssemblyAI、ElevenLabs、ローカル OpenAI-compatible、ローカル `whisper.cpp`
...
現在のテスト状況：**29 個のテストファイル、266 個のテストケースがすべてパス**。プロバイダー設定、文字起こし状態/安定化、字幕エクスポート、セッションライフサイクルとリポジトリ、ストレージ、クラウドバックアップ、Open API IPC 応答、AI 後処理パース、文字起こしテキストソース解決をカバーしています。
