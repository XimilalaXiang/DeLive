...
```bash
# 現在のプラットフォーム用にビルド
npm run dist:win       # Windows: .exe インストーラー + ポータブル版
...
1. **システム要件** - Windows 10+、macOS 13+（Ventura）、Linux（Ubuntu 20.04+ または同等）
...
6. **macOS オーディオ** - システム音声キャプチャには macOS 13+（ScreenCaptureKit）が必要
7. **Linux オーディオ** - システム音声ループバックキャプチャには PulseAudio が必要
8. **自動起動** - Windows と macOS のみサポート
9. **自動更新** - Windows、macOS、Linux AppImage をサポート

### 🛡️ Windows SmartScreen 警告について

DeLiveを初めて実行すると、Windowsが「WindowsによってPCが保護されました」というSmartScreen警告を表示する場合があります。これはMicrosoftでまだ評判が確立されていない新しいアプリケーションの**正常な動作**です。

**なぜこの警告が表示されるのか？**
- DeLiveは有料のコード署名証明書を持たないオープンソースプロジェクトです
- 広く使用されていない新しいアプリケーションはこの警告をトリガーします
- これはソフトウェアが有害であることを意味**しません**

**続行方法：**
1. 警告ダイアログで **「詳細情報」** をクリック
2. **「実行」** をクリックしてDeLiveを起動

**安全性の確認：**
- [VirusTotal スキャン結果](https://www.virustotal.com/gui/file/cdc1680fd693ac7b1c08980e8af5b04edf42289a051f9e7ecd4d915db9bce24b/detection) - アプリケーションが安全であることを確認できます
- ソースコードはGitHubで完全にオープンで監査可能です
