...
- **トピック機能**。セッションを絵文字と説明付きのトピックコンテナで整理。トピック内のセッションはデフォルトの Review 一覧からは分離されるが、グローバル検索では見つかる。Live から直接トピックへ録音するか、既存セッションをトピックへ移動可能。
- **ローカルファースト永続化**。セッション、タグ、トピック、設定は IndexedDB / localStorage に保存。秘密情報は OS 対応時に Electron `safeStorage` で暗号化保存。
...
- ナビゲーションバーから **Topics** タブを開く。
- トピックを作成：名前、絵文字、任意で説明を設定。
- 録音開始は 2 通り：トピックカード上の **新規録音** をクリックするか、Live ビューで **トピックを選択** してから録音。
- 録音ボタン上に選択中のトピックバッジが表示される。
- Review の Overview タブから、セッションをトピックへ追加・削除して移動可能。
- トピック内のセッションはデフォルトの Review 一覧には表示されないが、グローバル検索では見つかる。
