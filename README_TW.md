...
| **Mistral AI** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | Voxtral Realtime，經 `wss://api.mistral.ai`；代理注入 `Authorization` Header |
| **Deepgram** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | Nova-3 / Nova-2 串流 ASR；代理注入 `Authorization: Token` Header |
| **AssemblyAI** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | Universal-3 Pro 串流；最適合英語和多語言內容 |
| **ElevenLabs** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | Scribe v2 Realtime；90+ 種語言含中文；音訊以 base64 JSON 傳送 |
| **本地 OpenAI-compatible** | 本地服務 | 視窗批次處理重轉寫 | `MediaRecorder` (`webm/opus`) → `/v1/audio/transcriptions` | 適配 Ollama 或其他相容閘道，支援服務/模型偵測和可選 Ollama 拉取 |
