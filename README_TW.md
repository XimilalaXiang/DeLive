...
- [x] **12 條 ASR 路徑統一在一個 UI 裡** — Soniox、火山引擎、ElevenLabs、Mistral AI、Gladia、Deepgram、AssemblyAI、Cloudflare Workers AI、矽基流動、Groq、本地 OpenAI-compatible、本地 `whisper.cpp`
...
- [x] **AI 複盤工作台（Review Desk）** — 全頁檢視，動畫標籤列導覽（Transcript、AI 糾錯、Overview、AI 分析、Chat、Mind Map）
- [x] **AI 轉錄糾錯** — 雙模式：直接糾錯（流式輸出糾正結果）和先偵測後糾錯（先偵測問題，使用者確認後再糾正）；並排對比檢視；糾錯文本匯出
- [x] **AI 後處理智慧文本源選擇** — 有糾錯文本時自動使用；支援使用者偏好設定（自動 / 始終原始 / 始終糾錯後），各 AI 標籤頁即時狀態橫幅
...
- [x] **檔案轉錄** — 上傳音訊/影片檔案，使用 10 種 ASR 引擎離線轉錄為帶時間戳的文本，支援說話人分離和多語言偵測
...
| Provider | 類型 | 傳輸模式 | 音訊路徑 | 檔案轉錄 | 亮點 |
|----------|------|----------|----------|----------|------|
| **Soniox V4** | 雲端 | 即時流式 | `MediaRecorder` (`webm/opus`) → WebSocket | 支援 | token 級即時轉錄、即時翻譯、雙語字幕、多發言人辨識 |
| **火山引擎** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | 中文場景友善；代理在 Electron 側補齊必要 Header |
| **ElevenLabs** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | Scribe v2 Realtime；99 種語言含中文；音訊以 base64 JSON 傳送 |
| **Mistral AI** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | Voxtral Realtime，經 `wss://api.mistral.ai`；代理注入 `Authorization` Header |
| **Gladia** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | Solaria-1 即時；100+ 種語言；<300ms 延遲；代理處理會話初始化和 `x-gladia-key` 鑑權 |
| **Deepgram** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | Nova-3 / Nova-2 串流 ASR；代理注入 `Authorization: Token` Header |
| **AssemblyAI** | 雲端 | 即時流式 | `AudioWorklet` PCM16 → 內建代理 → WebSocket | 支援 | Universal-3 Pro 串流；最適合英語和多語言內容 |
| **Cloudflare Workers AI** | 雲端 | 視窗批次處理重轉寫 | `AudioWorklet` PCM16 → WAV → REST | 支援 | 基於 Whisper；低成本含免費額度；VAD 過濾和反幻覺 |
| **矽基流動** | 雲端 | 視窗批次處理重轉寫 | `AudioWorklet` PCM16 → WAV → REST | 支援 | SenseVoice、TeleSpeech、Qwen Omni 等模型路徑 |
| **Groq** | 雲端 | 視窗批次處理重轉寫 | `AudioWorklet` PCM16 → WAV → REST | 支援 | Whisper `large-v3-turbo` / `large-v3` 準即時會話更新 |
...
- 任意一種 Provider 路徑：
  ...
  - **ElevenLabs**：[elevenlabs.io](https://elevenlabs.io) 的 API Key
  - **Mistral AI**：[mistral.ai](https://mistral.ai) 的 API Key
  - **Gladia**：[gladia.io](https://app.gladia.io) 的 API Key
  - **Deepgram**：[deepgram.com](https://deepgram.com) 的 API Key
  - **AssemblyAI**：[assemblyai.com](https://www.assemblyai.com) 的 API Key
  - **Cloudflare Workers AI**：[dash.cloudflare.com](https://dash.cloudflare.com) 的 API Token 和 Account ID
...
`npm run dev` 會同時啟動 Vite 和 Electron。Provider 代理（火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）已內建在 Electron 主程序中，正常桌面開發不需要單獨後端。
...
- **AI 糾錯標籤頁**：雙模式糾錯（直接糾錯 / 先偵測後糾錯），流式輸出帶即時進度，並排對比檢視，糾錯文本匯出 TXT / Markdown
- **Overview 標籤頁**：AI briefing — 摘要、行動項、關鍵詞、章節、標題/標籤建議，一鍵套用
- **AI 分析標籤頁**：詳細 AI 分析，帶文本源狀態橫幅顯示目前使用原始或糾錯後文本
...
| Provider 抽象層 | `frontend/src/providers/registry.ts`, `frontend/src/providers/implementations/*` | 把 12 個後端統一到同一套 contract 和 capability 模型。 |
...
| Review Desk UI | `frontend/src/components/review/SessionTabBar.tsx`, `frontend/src/components/review/SessionHeader.tsx`, `frontend/src/components/review/OverviewTab.tsx`, `frontend/src/components/review/TranscriptTab.tsx`, `frontend/src/components/review/CorrectionTab.tsx`, `frontend/src/components/review/AiTab.tsx`, `frontend/src/components/review/ChatTab.tsx`, `frontend/src/components/review/MindMapTab.tsx`, `frontend/src/components/review/TextSourceBanner.tsx`, `frontend/src/components/review/MarkdownRenderer.tsx` | 動畫標籤列（含鍵盤導覽）、會話標頭（多格式匯出 TXT/Markdown/SRT/VTT）、AI 糾錯流式對比、文本源狀態橫幅、各標籤頁檢視、GFM Markdown 渲染（含語法高亮）和思維導圖編輯。 |
...
| 共享契約層 | `shared/electronApi.ts`, `electron/preload.ts`, `shared/volcProxyCore.ts`, `shared/mistralProxyCore.ts`, `shared/deepgramProxyCore.ts`, `shared/assemblyaiProxyCore.ts`, `shared/elevenlabsProxyCore.ts`, `shared/gladiaProxyCore.ts` | 定義 renderer 與 main 的型別化橋接介面，以及各 Provider 代理共享協議輔助邏輯。 |
| 除錯與發佈支援 | `server/`, `scripts/`, `.github/workflows/release.yml`, `.github/workflows/ci.yml`, `.github/workflows/test-build.yml` | 獨立代理除錯、圖示/執行階段預置指令碼、持續整合、測試構建和 tag 觸發的多平台構建發佈。 |
...
| Provider 層 | 註冊表 + 12 個實作 | 同時涵蓋即時雲端、視窗批次處理雲端、本地服務和本地 runtime。 |
| Electron 服務 | 內建多 Provider 代理（火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia）、本地 runtime 控制器、safe-storage IPC、diagnostics IPC | 提供瀏覽器環境無法直接完成的能力。 |
...
2. **Provider 代理**：正常桌面使用無需單獨後端，Electron 內建了火山引擎、Mistral、Deepgram、AssemblyAI、ElevenLabs、Gladia 的 WebSocket 代理。
